  ...
  (let* ((move-name (concatenate 'string (symbol-name name) "-MOVE"))
	...
	(format stream
		 ...
		 (list ,@(mapcar (lambda (param-name)
				   `(format nil
				   		;; Encadrer le second paramètre avec des "" si c'est du texte
					    ":~a ~a"
    ...
    ;;              (let* ((param-value (funcall param-name move))  ;; Récupère la valeur du paramètre
    ;;                     (formatted-value (if (stringp param-value)
    ;;                                          (format nil "\"~a\"" param-value)  ;; Ajoute des guillemets si c'est une chaîne
    ;;                                          param-value)))  ;; Laisse intact sinon
    ;;                (format nil ":~a ~a" (string-upcase (symbol-name param-name)) formatted-value)))
    ;;            parameters-names)))
...
(defun format-moves (moves)
  ...
  (format nil "~{~4@T~A~^~%~}" (mapcar #'move-format moves))) ;;4 spaces before move string
...
of MOVE, print the formatted result to the console, and return the formatted string."
  (let ((slot-names (mapcar #'sb-mop:slot-definition-name (sb-mop:class-slots (class-of move)))))
    (let ((formatted-string
           (apply #'format
                  nil   ;; Le premier argument 'nil' capture la chaîne formatée
                  format-string
                  (mapcar (lambda (slot-name) (slot-value move slot-name)) slot-names))))
      formatted-string)))
